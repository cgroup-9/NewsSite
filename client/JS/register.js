$(document).ready(() => {
    function isDevEnv() {    //check the server
        return location.host.includes('localhost');  
    }

    const port = 7019;
    const baseApiUrl = isDevEnv()   //check  if local host or ruppin server
        ? `https://localhost:${port}`  
        : "https://proj.ruppin.ac.il/cgroup9/test2/tar1";
    const url = `${baseApiUrl}/api/Users`;

    const divAddUser = $("#registerContainer");  //dinamic html
    ...
    $("#submitRegister").click(() => {    //catch information after submit
        const userToSend = {
